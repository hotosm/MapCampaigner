...
def get_mock_user(username, email):
    mock_user = User(
        osm_user_id=username, email=email)
...
class TestUser(TestCase):
    ...
    def test_01_user_name(self):
        mock_user = get_mock_user("user", "email@gmail.com")
...
def get_mock_campaign(creator_id, name, description,
                      start_date, end_date, create_on):
    mock_campaign = Campaign(creator_id=creator_id,
                             name=name,
...
class TestCampaign(TestCase):
    ...
    def test_01_campaign_name(self):
        import datetime
        mock_user = get_mock_user("user", "email@gmail.com")
        mock_campaign = get_mock_campaign(mock_user.id, "campaign",
    ...
    def test_04_geom_campaign(self):
        ...
        mock_campaign = session.query(Campaign).first()
        mock_taskBoundary = TaskBoundary(coordinates=geom_obj,
                                         name="name",
                                         status="status",
...
def get_mock_featureType(feature, name):
    mock_feature_type = FeatureType(feature=feature,
                                    name=name)
...
class TestFeatureType(TestCase):
    ...
    def test_01_featuretype_length(self):
        mock_feature_type = get_mock_featureType("feature",
                                                 "feature_name")
...
def get_feature_template(featureType):
    template = FeatureTemplate(name="Template",
...
class TestFeatureTemplate(TestCase):
    ...
    def test_01_is_template(self):
        featureType = get_mock_template_featureType("feature",
                                                    "feature_nane", True)
...
def get_mock_task_boundary(coordinates, name, status, type_boundary,
                           campaign_id):
    mock_taskBoundary = TaskBoundary(coordinates=coordinates,
                                     name=name,
                                     status=status,
                                     type_boundary=type_boundary,
...
class TestTaskBoundary(TestCase):
    def test_01_create_task_boundary(self):
        ...
        mock_task_boundary = get_mock_task_boundary(geom_obj, "Area1",
                                                    "inactive", "Feature",
                                                    mock_campaign.id)
        mock_task_boundary =\
            session.query(TaskBoundary.coordinates.ST_AsGeoJSON()
                          ).first()
...
class TestTeam(TestCase):
    ...
    def test_01_create_team(self):
        ...
        mock_task_boundary = get_mock_task_boundary(geom_obj, "Area1",
                                                    ...
                                                    mock_campaign.id)
        mock_task_boundary = session.query(TaskBoundary).first()
        mock_team = create_mock_team("Team7", mock_task_boundary.id)
...
def get_mock_function(name, feature, type_id):
    mock_function = Function(name=name, feature=feature, type_id=type_id)
...
class TestFunction(TestCase):
    ...
    def test_01_create_function(self):
        featureType = get_mock_template_featureType("feature",
                                                    "feature_nane", True)
        mock_function = get_mock_function("Function1",
                                          "data_collection",
                                          featureType.id)
